...
def crear_usuario(conn, nombre_usuario, password_hash, email, comunidad_autonoma, especialidad, rol='usuario', activo=True):
    try:
        with conn.cursor() as cursor:
            sql_insert = """
            INSERT INTO usuarios (nombre_usuario, email, password_hash, comunidad_autonoma, especialidad, rol, activo)
        ...
        if "usuarios_nombre_usuario_key" in str(e).lower():
            st.error(f"Error: El nombre de usuario '{nombre_usuario}' ya está registrado.")
        elif "usuarios_email_key" in str(e).lower():
...
def obtener_usuario_por_nombre(conn, nombre_usuario):
    try:
        with conn.cursor() as cursor:
            sql_select = """
            SELECT id, nombre_usuario, email, password_hash, comunidad_autonoma, especialidad, rol, activo, fecha_registro
...
def obtener_usuario_por_email(conn, email):
    try:
        with conn.cursor() as cursor:
            sql_select = "SELECT id, nombre_usuario, email, password_hash, comunidad_autonoma, especialidad, rol, activo FROM usuarios WHERE email = %s;"
